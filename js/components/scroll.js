...
export default class ScrollCtrl {
	...
	goTo(id, delay) {
		...
		const section =
			Util.isElement(id) || isNumber ? id : document.querySelector(id);
		if (section || isNumber) {
	...
	updateSections(sections) {
		...
		for (var i = 0; i < this.sectionsL; i++) {
			...
			if (s.$el == undefined) s.$el = document.querySelector(s.selector);
			...
			if (isSticky) {
				s.sticky = true;
				s.stickyPlaceholder = s.$el.querySelector(
					"[data-scroll-sticky-placeholder]"
				);
				s.stickyContent = s.$el.querySelector("[data-scroll-sticky-content]");
				s.stickyContainer = s.$el.querySelector(
					"[data-scroll-sticky-container]"
				);
			}
		}
		this.resize();
	...
	touchMove() {
		...
		const someSlider = this.sliders.some((slider) =>
			Util.isInViewportDom(slider)
		);
	...
	mouseWheel() {
		this.pausePointer = false;
		var t = this.event.wheelDeltaY
			? this.event.wheelDeltaY
			: this.event.wheelDelta;
		if (this.onlyVirtual) {
			if (t < 0)
				t = -Math.min(Math.abs(t * 0.7), Math.abs(this.maxDelta * 0.4));
			else t = Math.min(Math.abs(t * 0.7), Math.abs(this.maxDelta * 0.4));
		}
		this.delta = t;
	...
	wheel() {
		...
		if (this.isFirefox && this.event.deltaMode) t *= 60;
		if (this.isIE) t *= 2;
		t *= 0.554;
		const someSlider = this.sliders.some((slider) =>
			Util.isInViewportDom(slider)
	...
	keyDown() {
		...
		const isHome =
			name === "Home" || (this.event.metaKey && name === "ArrowUp");
		const isEnd =
			name === "End" || (this.event.metaKey && name === "ArrowDown");
		if (isHome || isEnd) {
	...
	update() {
		if (this.onlyVirtual)
			this.target =
				this.maxVirtual === 0
					? Util.round(Math.min(this.target, 0))
					: Util.round(Math.max(Math.min(this.target, 0), -this.maxVirtual));
		else
	...
	addParalax(fun) {
		if (this.paralax.indexOf(fun) === -1) this.paralax.push(fun);
	}
	removeParalax(fun) {
	...
	updateElements(target) {
		...
		for (var i = 0; i < this.sectionsL; ++i) {
			...
			if (section.sticky) {
				if (
					target > section.rect.top + this.winH &&
					target < section.rect.top + section.stickyHeight
				) {
					...
					if (target < section.rect.top + section.stickyHeight)
						...
						this.translate(
							section.stickyContainer,
							-(section.stickyHeight - this.winH),
							i
						);
				}
			}
	...
	resize() {
		...
		for (var i = 0; i < this.sectionsL; ++i) {
			...
			if (section.sticky) {
				let height = section.stickyContent.getAttribute(
					"data-scroll-sticky-height"
				)
					? Number(
							section.stickyContent.getAttribute("data-scroll-sticky-height")
					  ) * this.winH
					: section.stickyContent.scrollHeight + this.winH;
				const heightMultiply = section.stickyContent.getAttribute(
					"data-scroll-sticky-multiply"
				)
					? Number(
							section.stickyContent.getAttribute("data-scroll-sticky-multiply")
					  )
					: 1;
