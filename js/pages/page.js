...
import "../components/molecules/asideTabs.js";
...
import "../components/molecules/documentsSlider.js";
...
export default class Page {
	...
	paralax() {
		...
		this.scrollItems.forEach((item) => {
			const offset = item.getAttribute("data-scroll-offset")
				? Number(item.getAttribute("data-scroll-offset"))
				: 120;
			if (
				!item.classList.contains("show") &&
				Util.isInViewportDom(item, offset)
			) {
				item.classList.add("show");
			} else if (
				item.classList.contains("show") &&
				item.getBoundingClientRect().top > this.winH
			) {
